const fetch = require('node-fetch');
  ...
  async function makePhotoEdit(imageUrl, prompt, model) {
      ...
      for (const apikey of keys) {
          try {
              const params = new URLSearchParams({ url: imageUrl, prompt, apikey, model: usedModel, quality: '4k' });
              const res = await fetch(`${BASE}/tools/photoeditorv3?${params}`, { timeout: 120000 });
              if (!res.ok) { lastErr = new Error(`HTTP ${res.status}`); continue; }
              const data = await res.json();
              if (!data.success || !data.result) { lastErr = new Error(data.message || 'no result'); continue; }
              const imgUrl = data.result?.output || data.result?.image_url || data.result?.all_outputs?.[0];
              if (!imgUrl || typeof imgUrl !== 'string') { lastErr = new Error('no output url'); continue; }
              return imgUrl;
          } catch (e) { lastErr = _cleanErr(e); }
      }
