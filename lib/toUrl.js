    ...
    try {
      ...
      try {
          const res = await axios.put(`https://transfer.sh/${filename}`, buf, {
              headers: { 'Content-Type': 'application/octet-stream', 'Max-Days': '7' },
              timeout: 25000
          });
          const url = typeof res.data === 'string' ? res.data.trim() : '';
          if (url.startsWith('http')) return url;
          ...
          errors.push('transfer.sh: ' + e.message);
      }

      try {
          const form = new FormData();
          form.append('f', buf, { filename, contentType: 'application/octet-stream' });
          const res = await axios.post('https://oshi.at', form, {
              headers: form.getHeaders(),
              timeout: 20000
          });
          const txt = typeof res.data === 'string' ? res.data : '';
          const m = txt.match(/https?:\/\/oshi\.at\/\S+/);
          if (m) return m[0];
          throw new Error('no url');
      } catch (e) {
          errors.push('oshi.at: ' + e.message);
      }
