...
module.exports = async (context) => {
    await ownerMiddleware(context, async () => {
        ...
        const buttons = [
            { buttonId: `${prefix}antilink on`, buttonText: { displayText: 'Turn ON 🔗' }, type: 1 },
            { buttonId: `${prefix}antilink off`, buttonText: { displayText: 'Turn OFF 🚫' }, type: 1 },
        ...
        if (value === 'on' || value === 'off') {
            ...
            if (isEnabled === action) {
                message = formatStylishReply(
                    `Antilink is already ${value.toUpperCase()}! 😈\n\n` +
                    `Status: ${isEnabled ? '✅ ON' : '❌ OFF'}\n` +
                    `Use "${prefix}antilink on" or "${prefix}antilink off" to toggle.`
                );
            } else {
                await updateSetting('antilink', action ? 'true' : 'false');
                message = formatStylishReply(
                    `Antilink ${value.toUpperCase()} activated! 🔥\n\n` +
