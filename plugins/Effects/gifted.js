const { giftedEffect } = require('../../lib/giftedApi');

const GIFTED_EFFECTS = [
...
const built = [];
for (const eff of GIFTED_EFFECTS) {
    built.push({
        name: eff.name,
        aliases: eff.aliases || [],
        ...
        run: (function(effect) {
            return async (context) => {
                ...
                try {
                    ...
                    const imgBuffer = await giftedEffect(effect.id, text.trim(), effect.extraParams || {});
                    await client.sendMessage(m.chat, { react: { text: '✅', key: m.key } });
