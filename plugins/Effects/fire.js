...
module.exports = {
    name: 'fire',
    alias: ["firetext","flame","flametext"],
    description: 'Generate fire text image effect',
    run: async (context) => {
        const { client, m, text, prefix } = context;
        if (!text) return m.reply(`╭───(    TOXIC-MD    )───\n├───≫ FIRE TEXT ≪───\n├ \n├ Usage: ${prefix}fire YourText\n├ Max 30 characters.\n╰──────────────────☉
> ©𝐏𝐨𝐰𝐞𝐫𝐞𝐝 𝐁𝐲 𝐱𝐡_𝐜𝐥𝐢𝐧𝐭𝐨𝐧`);
        if (text.length > 30) return m.reply(`╭───(    TOXIC-MD    )───\n├ Text too long. Max 30 chars.\n╰──────────────────☉
> ©𝐏𝐨𝐰𝐞𝐫𝐞𝐝 𝐁𝐲 𝐱𝐡_𝐜𝐥𝐢𝐧𝐭𝐨𝐧`);
        try {
            await client.sendMessage(m.chat, { react: { text: '⌛', key: m.key } });
            const raw = await ephoto('https://en.ephoto360.com/create-fire-text-effect-online-813.html', text.trim());
            let imgBuffer;
