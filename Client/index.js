const {
  ...
  proto,
  getContentType,
} = require("baileys-pro"); // Change to baileys-elite if needed
...
const autolike = require("../Functions/autolike"); // New autolike function
authenticationn();
...
async function startToxic() {
  ...
  client.ev.on("messages.upsert", async (chatUpdate) => {
    ...
    try {
      ...
      // Call autolike function
      await autolike(client, mek);

      // Skip protocol messages
      if (mek.message.protocolMessage) {
        console.log(chalk.yellow(`Skipped protocol message in ${mek.key.remoteJid}`));
        return;
      }
