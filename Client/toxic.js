...
module.exports = toxic = async (client, m, chatUpdate, store) => {
    try {
        ...
        if (!m.chat) {
            console.log("Invalid chat JID, skipping command execution");
            return;
        }
        ...
        if (cmd) {
            ...
            if (mode === 'private' && !itsMe && !Owner && !sudoUsers.includes(senderNumber)) {
                return;
            }
        ...
        console.log("Error in toxic.js:", util.format(err));
    }
    ...
    process.on('uncaughtException', function (err) {
        ...
        console.log('Caught exception in toxic.js:', err);
    });
};
