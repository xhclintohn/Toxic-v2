    ...
    try {
      ...
      const toFancyFont = (text, isUpperCase = false) => {
        const fonts = {
          ...
          'a': '𝙖', 'b': '𝙗', 'c': '𝙘', 'd': '�', 'e': '𝙚', 'f': '𝙛', 'g': '𝙜', 'h': '𝙝', 'i': '𝙞', 'j': '𝙟', 'k': '𝙠', 'l': '𝙡', 'm': '𝙢',
      ...
      const audioUrl = 'https://drive.google.com/uc?export=download&id=1aCgY0Nj12mfgluM7Cy_BdafXK1vtBPRZ';

      try {
        console.log(`✅ Attempting to fetch and send audio from Google Drive: ${audioUrl}`);
        await client.sendMessage(m.chat, {
          audio: { url: audioUrl },
          ptt: true, // Ensures voice note interface
          mimetype: 'audio/mpeg', // Assuming MP3 for simplicity
          fileName: 'menu.mp3' // Consistent with MP3
        }, { quoted: m });
        console.log(`✅ Audio sent successfully as voice note`);
      } catch (error) {
        console.error('❌ Error fetching or sending voice note from Google Drive:', error.message);
        await client.sendMessage(m.chat, {
